...
The system employs a decentralized three-tier topology to decouple high-frequency physical sensing, event orchestration, and human telemetry visualization.
...
| **TVS diode protection**        | Suppress voltage transients on the relay coil flyback and AC input           | Bidirectional TVS (P6KE250CA) across relay contacts; unidirectional TVS on 3.3V rail for ESD protection                                                                                            |
| **Fused AC input**              | Overcurrent protection upstream of all circuitry                             | 5A fast-blow ceramic fuse (5×20mm) on live conductor before relay and CT sensor tap                                                                                                                |

### Schematic Hierarchy

```
┌─────────────────────────────────────────────────────────────────────┐
│                        230V AC MAINS DOMAIN                         │
│                                                                     │
│   [L] ──── Fuse ──┬──── SCT-013 CT ──── [Load Appliance] ──── [N] │
│                    │                                                │
│                    └──── Relay Contact (NO) ────┘                   │
│                                                                     │
├─────────────── ISOLATION BARRIER (Optocoupler) ─────────────────────┤
│                                                                     │
│                        3.3V LOGIC DOMAIN                            │
│                                                                     │
│   [ESP32 GPIO] ──── Optocoupler LED ──── Relay Driver (NPN)        │
│   [ESP32 I2C] ──── ADS1115 (16-bit ADC) ──── Burden Resistor ──── CT Secondary              │
│   [ESP32 UART] ──── Debug / OTA Flash                              │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
