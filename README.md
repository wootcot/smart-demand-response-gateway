...
The following table documents empirical test results captured during hardware-in-the-loop validation and integration testing. Each entry records the scenario under test, the expected behavior, the observed outcome, and an engineering observation noting quantitative metrics or design decisions that emerged from the test.

| Test Scenario                                   | Expected Result                                                                 | Actual Result                                                               | Engineering Observation                                                                                                                                                                                   |
| ----------------------------------------------- | ------------------------------------------------------------------------------- | --------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| WebSocket round-trip latency under nominal load | Command acknowledgment received within 200ms                                    | ACK received in 120ms average over 50 trials                                | 120ms latency via WebSocket communication confirms suitability for sub-second demand response actuation                                                                                                   |
| Inrush current spike from water heater startup  | Gateway should not trigger false load-shed event during transient spike         | Initial firmware triggered false positive on 18A inrush spike lasting ~1.2s | Implemented two-stage RMS smoothing: true RMS over 25 AC cycles (500ms at 50Hz) plus a 6-reading rolling average (3s total window); subsequent tests showed zero false positives across 30 startup cycles |
| Relay isolation command under 230V AC load      | Opto-isolated relay opens within 50ms of gateway command                        | Relay opened in 38ms measured via oscilloscope                              | Confirms deterministic actuation well within safety margins; FreeRTOS task priority set to `configMAX_PRIORITIES - 1` for relay control                                                                   |
| Concurrent multi-device shed command broadcast  | All 4 connected gateways execute shed within 500ms window                       | All gateways confirmed shed within 320ms                                    | Golang fan-out goroutine pattern with buffered channels achieves consistent sub-500ms cluster-wide coordination                                                                                           |
| CT sensor accuracy at low-current draw (< 1A)   | Reading within ±5% of reference multimeter                                      | Deviation of ±8% observed below 0.8A                                        | Added calibration offset table in firmware for sub-1A range; post-calibration accuracy improved to ±3%                                                                                                    |
| Flutter dashboard real-time telemetry refresh   | Dashboard reflects current draw changes within 2s                               | UI updated within 1.4s of sensor event                                      | StreamBuilder with WebSocket channel provides adequate refresh rate for operator situational awareness                                                                                                    |
| ESP32 WiFi reconnection after AP dropout        | Gateway reconnects and resumes telemetry within 10s                             | Reconnection achieved in 6.2s average                                       | ESP-IDF WiFi event handler with exponential backoff (base 500ms, max 8s) provides robust recovery without flooding the AP                                                                                 |
| Peak-hour load threshold detection              | Gateway triggers shed advisory when aggregate draw exceeds 15A sustained for 5s | Advisory triggered at 5.1s after threshold breach                           | Hysteresis band of 0.5A added to prevent oscillation near threshold boundary                                                                                                                              |
